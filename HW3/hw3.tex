...
\begin{enumerate}[label=(\alph*)]
    ...
    $\log(1) + \cdots + \log(1) < \log(1) + \log(2) + \cdots + \log(n) < \log(n) + \cdots + \log(n) \rightarrow 0 < \log(n!) < \log(n^n)$
...
def Search(low, high, $A$):
    ...
    \tabto{20pt} if $(\text{low} == \text{high})$:

        \tabto{40pt}if $(A[\text{low}] == \text{low})$:

            \tabto{60pt}return low

        \tabto{40pt}else$:$

            \tabto{60pt}return False

    \tabto{20pt}else$:$

        \tabto{40pt}mid = $(\text{low} + \text{high})//2$

        \tabto{40pt}if $(A[\text{mid}] == \text{mid})$:

            \tabto{60pt}return mid

        \tabto{40pt}elif $(A[\text{mid}] > \text{mid}):$

            \tabto{60pt}return Search$(\text{low}, \text{mid}-1, A)$

        \tabto{40pt}else$:$

            \tabto{60pt}return Search$(\text{mid}+1, \text{high}, A)$

    
    $\underline{\text{Run-time Analysis}: O(\log(n))}$
